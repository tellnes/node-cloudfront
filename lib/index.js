  ...
  if (config.trustedSigners) {
      ...
      data.TrustedSigners = trustedSigners.map(function(signer){
          if (signer == 'self') {
              ...
              return {AwsAccountNumber: signer}
          }
      });
  }
...
CloudFront.prototype.setDistributionConfig = function(distribution, config, callerReference, cb) {
  ...
  var self = this;
  var req = this.request('PUT', (config.streaming ? 'streaming-' : '') + 'distribution/' + distribution + '/config', {'If-Match': config.etag}, function(err, body, res) {
    if (err) return cb(err);
  ...
  var packet = this.generateDistributionConfig(config);
  console.log(packet);
  req.write(packet);
  req.end();
};
...
CloudFront.prototype.createOriginAccessIdentity = function(callerReference, comment, cb) {
    ...
    req.write(data2xml('CloudFrontOriginAccessIdentityConfig', {
        CallerReference: callerReference,
        Comment: comment || ''
    }));
    req.end();
...
CloudFront.prototype.listOriginAccessIdentities = function(cb) {
    this.request('GET', 'origin-access-identity/cloudfront', function(err, body) {
        if (err) return cb(err);
...
CloudFront.prototype.signUrlWithCannedPolicy = function(urlString, expires, privateKey, keyPairId) {
    ...
    //what if it requires strict match? e.g. "Statement"
    ...
    urlObject.query.Expires = expires;
    urlObject.query.Signature = signature.replace(/\+/g, '-').replace(/\//g, '~').replace(/=/g, '_');
    urlObject.query['Key-Pair-Id'] = keyPairId;
